...
export default {
    ...
    methods:{
        head_bg(picSrc){
            return `background-image: url(${picSrc});`
        },
        initScroll(){
            //使用了better-scroll，默认它会阻止touch事件。所以在配置中需要加上click: true
        ...
        selectMenu(index){
            ...
            //滚动到对应元素的位置
            this.foodScroll.scrollToElement(element,0);
        },
        calculateCount(spus){
            let count = 0;
            spus.forEach(food =>{
        ...
        .then(response =>{
            if(response.code == 0){
                ...
                this.$nextTick(() =>{
                    ...
                    //监听滚动位置   ======>在initScroll中实现
                    //根据滚动位置确定下标，与左侧对应  ======>通过计算属性currentIndex实现
                    //通过下标实现点击左侧，滚动右侧 
                })
            }
        })
